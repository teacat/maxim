...
func (b *Bucket) Write(msg string) error {
	for _, v := range b.sessions {
		err := v.Write(msg)
		if err != nil {
			return err
...
func (b *Bucket) WriteFilter(msg string, fn func(*Session) bool) error {
	for _, v := range b.sessions {
		if fn(v) {
			v.Write(msg)
		}
...
func (b *Bucket) WriteOthers(msg string, s *Session) error {
	for _, v := range b.sessions {
		if v != s {
			err := v.Write(msg)
			if err != nil {
...
func (b *Bucket) WriteBinary(msg []byte) error {
	for _, v := range b.sessions {
		err := v.WriteBinary(msg)
		if err != nil {
			return err
		}
	}
...
func (b *Bucket) WriteBinaryFilter(msg []byte, fn func(*Session) bool) error {
	for _, v := range b.sessions {
		if fn(v) {
			v.WriteBinary(msg)
		}
...
func (b *Bucket) WriteBinaryOthers(msg []byte, s *Session) error {
	for _, v := range b.sessions {
		if v != s {
			v.WriteBinary(msg)
		}
